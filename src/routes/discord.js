...
router.put('/config', async (req, res) => {
  try {
    if (!soAdminGestor(req, res)) return;
    const { ativo, webhook_url, ev_ciencia, ev_pop, ev_processo, ev_aniversario, ev_comunicado } = req.body;
    const b = v => (v ? 1 : 0);
    await run(
      `INSERT INTO integracao_discord (empresa_id, webhook_url, ativo, ev_ciencia, ev_pop, ev_processo, ev_aniversario, ev_comunicado, atualizado_em)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8, NOW())
       ON CONFLICT (empresa_id) DO UPDATE SET
         webhook_url = EXCLUDED.webhook_url, ativo = EXCLUDED.ativo,
      ...
      [req.usuario.empresa_id, (webhook_url || '').trim() || null, b(ativo),
