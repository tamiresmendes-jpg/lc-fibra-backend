...
router.put('/config', async (req, res) => {
  try {
    if (!soAdminGestor(req, res)) return;
    const { ativo, sistema_url, ev_ciencia, ev_pop, ev_processo, ev_aniversario, ev_comunicado, ev_coffee, ev_mural, ev_cultura, ev_chatmix, canais_evento, servidor_id, canal_embed } = req.body;
    const b = v => (v ? 1 : 0);
    ...
    await run(
      `INSERT INTO integracao_discord (empresa_id, sistema_url, ativo, ev_ciencia, ev_pop, ev_processo, ev_aniversario, ev_comunicado, ev_coffee, ev_mural, ev_cultura, ev_chatmix, canais_evento, servidor_id, canal_embed, atualizado_em)
      ...
      [req.usuario.empresa_id, (sistema_url || '').trim() || null, b(ativo),
       b(ev_ciencia), b(ev_pop), b(ev_processo), b(ev_aniversario), b(ev_comunicado), b(ev_coffee), b(ev_mural), b(ev_cultura), b(ev_chatmix === undefined ? 1 : ev_chatmix), mapaJson, soDigitos(servidor_id), soDigitos(canal_embed)]
    );
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ erro: e.message }); }
});
