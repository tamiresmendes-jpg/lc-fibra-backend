...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const total = (await get("SELECT COUNT(*) as total FROM auditorias WHERE empresa_id=?", [eid])).total;
    const aprovadas = (await get("SELECT COUNT(*) as total FROM auditorias WHERE empresa_id=? AND status='aprovada'", [eid])).total;
    const rejeitadas = (await get("SELECT COUNT(*) as total FROM auditorias WHERE empresa_id=? AND status='rejeitada'", [eid])).total;
    const pendentes = (await get("SELECT COUNT(*) as total FROM auditorias WHERE empresa_id=? AND status='pendente'", [eid])).total;
    const solicitacoesPendentes = (await get("SELECT COUNT(*) as total FROM auditoria_solicitacoes WHERE empresa_id=? AND status='pendente'", [eid])).total;
    const mediaScore = (await get("SELECT AVG(score) as media FROM auditorias WHERE empresa_id=? AND score IS NOT NULL", [eid])).media;
    const porTipo = await all("SELECT tipo, COUNT(*) as total FROM auditorias WHERE empresa_id=? GROUP BY tipo", [eid]);
  ...
  try {
    ...
    if (itens && itens.length > 0) {
      for (const item of itens) {
        await run('INSERT INTO auditoria_itens (id, auditoria_id, pergunta, peso) VALUES (?, ?, ?, ?)', [uuidv4(), id, item.pergunta, item.peso || 1]);
      }
    }
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const itens = await all('SELECT * FROM auditoria_itens WHERE auditoria_id = ?', [req.params.id]);
...
router.delete('/:id', async (req, res) => {
  try {
    await run('DELETE FROM auditoria_itens WHERE auditoria_id=?', [req.params.id]);
    await run('DELETE FROM auditorias WHERE id=? AND empresa_id=?', [req.params.id, req.usuario.empresa_id]);
