...
router.post('/', autenticar, async (req, res) => {
  try {
    const { unidade, data, horario, titulo, observacao, imagem } = req.body;
    if (!unidade || !data) return res.status(400).json({ erro: 'Unidade e data são obrigatórios' });
    const id = uuidv4();
    await run(
      `INSERT INTO coffee_breaks (id, empresa_id, unidade, data, horario, titulo, observacao, imagem) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, req.usuario.empresa_id, unidade, data, horario || null, titulo || null, observacao || null, imagem || null]
    );
...
router.put('/:id', autenticar, async (req, res) => {
  try {
    ...
    const { unidade, data, horario, titulo, observacao, imagem } = req.body;
    await run(
      `UPDATE coffee_breaks SET unidade = ?, data = ?, horario = ?, titulo = ?, observacao = ?, imagem = ? WHERE id = ?`,
      [unidade, data, horario || null, titulo || null, observacao || null, imagem || null, req.params.id]
    );
