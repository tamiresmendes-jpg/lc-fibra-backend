...
router.get('/dashboard-stats', autenticar, async (req, res) => {
  try {
    ...
    const [acoes, metas, okrs, indicadores] = await Promise.all([
      all(`SELECT status, COUNT(*) AS n FROM acoes WHERE empresa_id=? AND excluido_em IS NULL GROUP BY status`, [eid]),
      all(`SELECT status, COUNT(*) AS n FROM metas WHERE empresa_id=? GROUP BY status`, [eid]),
      all(`SELECT status, COUNT(*) AS n FROM okrs WHERE empresa_id=? GROUP BY status`, [eid]),
      get(`SELECT COUNT(*) AS n FROM indicadores WHERE empresa_id=? AND status='ativo'`, [eid]),
    ]);
  ...
  try {
    const rows = await all(
      `SELECT m.*, d.nome AS departamento_nome, u.nome AS responsavel_nome
       ...
       WHERE m.empresa_id=? ORDER BY m.created_at DESC`,
...
router.post('/metas', autenticar, async (req, res) => {
  try {
    ...
    const id = uuidv4();
    await run(
      `INSERT INTO metas (id,empresa_id,titulo,descricao,valor_meta,valor_atual,unidade,departamento_id,responsavel_id,data_inicio,data_fim,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
    ...
    res.status(201).json(await get(`SELECT * FROM metas WHERE id=?`, [id]));
  } catch { res.status(500).json({ erro: 'Erro ao criar meta' }); }
});

router.put('/metas/:id', autenticar, async (req, res) => {
  try {
    const exist = await get(`SELECT id FROM metas WHERE id=? AND empresa_id=?`, [req.params.id, req.usuario.empresa_id]);
    if (!exist) return res.status(404).json({ erro: 'Não encontrado' });
    const { titulo, descricao, valor_meta, valor_atual, unidade, departamento_id, responsavel_id, data_inicio, data_fim, status } = req.body;
    await run(
      `UPDATE metas SET titulo=?,descricao=?,valor_meta=?,valor_atual=?,unidade=?,departamento_id=?,responsavel_id=?,data_inicio=?,data_fim=?,status=? WHERE id=?`,
    ...
    res.json(await get(`SELECT * FROM metas WHERE id=?`, [req.params.id]));
...
router.delete('/metas/:id', autenticar, async (req, res) => {
  try {
    const exist = await get(`SELECT id FROM metas WHERE id=? AND empresa_id=?`, [req.params.id, req.usuario.empresa_id]);
    if (!exist) return res.status(404).json({ erro: 'Não encontrado' });
    await run(`DELETE FROM metas WHERE id=?`, [req.params.id]);
    res.json({ ok: true });
  ...
  try {
    const rows = await all(
      `SELECT o.*, u.nome AS responsavel_nome FROM okrs o
       ...
       WHERE o.empresa_id=? ORDER BY o.created_at DESC`,
      [req.usuario.empresa_id]
    );
...
router.post('/okrs', autenticar, async (req, res) => {
  try {
    ...
    const id = uuidv4();
    await run(
      `INSERT INTO okrs (id,empresa_id,objetivo,resultados_chave,responsavel_id,data_inicio,data_fim,ciclo,status) VALUES (?,?,?,?,?,?,?,?,'ativo')`,
    ...
    res.status(201).json(await get(`SELECT * FROM okrs WHERE id=?`, [id]));
  } catch { res.status(500).json({ erro: 'Erro ao criar OKR' }); }
});

router.put('/okrs/:id', autenticar, async (req, res) => {
  try {
    const exist = await get(`SELECT id FROM okrs WHERE id=? AND empresa_id=?`, [req.params.id, req.usuario.empresa_id]);
    if (!exist) return res.status(404).json({ erro: 'Não encontrado' });
    const { objetivo, resultados_chave, responsavel_id, data_inicio, data_fim, ciclo, status } = req.body;
    await run(
      `UPDATE okrs SET objetivo=?,resultados_chave=?,responsavel_id=?,data_inicio=?,data_fim=?,ciclo=?,status=? WHERE id=?`,
    ...
    res.json(await get(`SELECT * FROM okrs WHERE id=?`, [req.params.id]));
