...
router.post('/', async (req, res) => {
  try {
    ...
    const id = uuidv4();
    await run(
      'INSERT INTO checklists (id,empresa_id,titulo,descricao,setor,itens) VALUES ($1,$2,$3,$4,$5,$6)',
      [id, eid(req), titulo, descricao||null, setor||null, itens||null]
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await run(
      ...
      [titulo, descricao||null, setor||null, itens||null, req.params.id, eid(req)]
    );
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});
